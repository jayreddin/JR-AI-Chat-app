document.addEventListener('DOMContentLoaded', function() {
    ...
    let terminalCallsEnabled = false;
    let textSize = 100;
    let tempUnit = 'C';
    let distanceUnit = 'km';
    ...
    let knowledgeBase = [];
    let systemPrompt = '';
    let reasoningModel = 'deepseek-reasoner';
    let reasoningPrompt = 'Step-by-step thinking';
    ...
    const aiFeaturesBtn = document.getElementById('ai-features-btn');
    ...
    // Initialize Puter environment indicator
    function initEnvironmentIndicator() {
        if (typeof puter !== 'undefined') {
            envType.textContent = puter.env;
            envIndicator.title = `Puter Environment: ${puter.env}`;
        } else {
            envType.textContent = 'web';
            envIndicator.title = 'Puter Environment: web';
        }
    }
    ...
    function loadUserPreferences() {
        ...
        // Load memory mode
        if (localStorage.getItem('memoryMode') === 'true') {
            memoryMode = true;
            document.getElementById('memory-mode-btn').classList.add('active');
        }
        
        // Load terminal calls
        if (localStorage.getItem('terminalCalls') === 'true') {
            terminalCallsEnabled = true;
            document.getElementById('terminal-calls-toggle').checked = true;
        }
        ...
        if (savedTextSize) {
            ...
            document.getElementById('text-size-input').value = textSize;
            document.documentElement.style.fontSize = textSize + '%';
        }
        
        // Load units
        tempUnit = localStorage.getItem('tempUnit') || 'C';
        document.querySelector('input[name="temp-unit"][value="' + tempUnit + '"]').checked = true;
        
        distanceUnit = localStorage.getItem('distanceUnit') || 'km';
        document.querySelector('input[name="distance-unit"][value="' + distanceUnit + '"]').checked = true;
        ...
        // Load tools
        const savedTools = localStorage.getItem('installedTools');
        if (savedTools) {
            installedTools = JSON.parse(savedTools);
        }
        
        const savedActivatedTools = localStorage.getItem('activatedTools');
        if (savedActivatedTools) {
            activatedTools = JSON.parse(savedActivatedTools);
        }
        
        // Load MCPs
        const savedMCPs = localStorage.getItem('installedMCPs');
        if (savedMCPs) {
            installedMCPs = JSON.parse(savedMCPs);
        }
        
        const savedActivatedMCPs = localStorage.getItem('activatedMCPs');
        if (savedActivatedMCPs) {
            activatedMCPs = JSON.parse(savedActivatedMCPs);
        }
        
        // Load agents
        const savedAgents = localStorage.getItem('installedAgents');
        if (savedAgents) {
            installedAgents = JSON.parse(savedAgents);
        }
        
        const savedActivatedAgents = localStorage.getItem('activatedAgents');
        if (savedActivatedAgents) {
            activatedAgents = JSON.parse(savedActivatedAgents);
        }
        ...
        reasoningModel = localStorage.getItem('reasoningModel') || 'deepseek-reasoner';
        document.getElementById('reasoning-model').value = reasoningModel;
        
        reasoningPrompt = localStorage.getItem('reasoningPrompt') || 'Step-by-step thinking';
    ...
    function saveUserPreferences() {
        ...
        localStorage.setItem('memoryMode', memoryMode);
        localStorage.setItem('terminalCalls', terminalCallsEnabled);
        ...
        localStorage.setItem('installedTools', JSON.stringify(installedTools));
        localStorage.setItem('activatedTools', JSON.stringify(activatedTools));
        localStorage.setItem('installedMCPs', JSON.stringify(installedMCPs));
        localStorage.setItem('activatedMCPs', JSON.stringify(activatedMCPs));
        localStorage.setItem('installedAgents', JSON.stringify(installedAgents));
        localStorage.setItem('activatedAgents', JSON.stringify(activatedAgents));
        localStorage.setItem('knowledgeBase', JSON.stringify(knowledgeBase));
        localStorage.setItem('reasoningModel', reasoningModel);
        localStorage.setItem('reasoningPrompt', reasoningPrompt);
        localStorage.setItem('reasoningEnabled', reasoningEnabled);
    }
    ...
    function initChat() {
        ...
        // Save empty conversation to local storage
        saveConversation(currentConversationId, 'New Conversation', [welcomeMessage]);
    }
    ...
    function addMessageToChat(message) {
        ...
        if (message.role === 'user') {
            ...
            const ttsMessageBtn = document.createElement('button');
            ttsMessageBtn.classList.add('tts-message-btn');
            ttsMessageBtn.innerHTML = '<i class="fas fa-volume-up"></i>';
            ttsMessageBtn.title = 'Read aloud';
            ttsMessageBtn.addEventListener('click', function() {
                speakText(message.content);
            });
            messageElement.appendChild(ttsMessageBtn);
        }
        ...
        // Add more buttons for AI messages
        if (message.role === 'assistant') {
            // Regenerate button
            const regenerateBtn = document.createElement('button');
            regenerateBtn.classList.add('message-action-btn');
    ...
    async function sendMessage() {
        ...
        thinkingModel.textContent = `Thinking: ${modelSelector.options[modelSelector.selectedIndex].text}`;
        
        // If reasoning is enabled, show reasoning process
        if (reasoningEnabled) {
            thinkingBox.style.display = 'block';
            thinkingText.textContent = 'Analyzing your request...';
            
            // Simulate reasoning process (replace with actual API call)
            simulateReasoning(message);
        }
        ...
        try {
            ...
            const aiResponse = await simulateAIResponse(message, messages);
            ...
            if (conversation.title === 'New Conversation' && messages.length >= 3) {
                const newTitle = await generateConversationTitle(messages);
                saveConversation(currentConversationId, newTitle, messages);
            } else {
                saveConversation(currentConversationId, conversation.title, messages);
            }
        } catch (error) {
    ...
    // Simulate AI response (replace with actual API call)
    async function simulateAIResponse(message, conversation) {
        ...
        if (message.toLowerCase().includes('hello') || message.toLowerCase().includes('hi')) {
            ...
            return 'I don\'t have real-time weather data, but I can help you find weather information. Which city are you interested in?';
    ...
    // Simulate reasoning process (replace with actual API call)
    async function simulateReasoning(message) {
        const reasoningSteps = [
            'Analyzing user query...',
            'Identifying key concepts in the message...',
    ...
    async function generateConversationTitle(messages) {
        ...
        // Simulate title generation (replace with actual AI call)
    ...
    // Show conversations in chat history modal
    function showConversations() {
        ...
        for (const conversation of sortedConversations) {
            ...
            const conversationTimestamp = document.createElement('div');
            conversationTimestamp.classList.add('conversation-timestamp');
            conversationTimestamp.textContent = formatRelativeTime(conversation.timestamp);
            
            conversationInfo.appendChild(conversationTitle);
            conversationInfo.appendChild(conversationTimestamp);
            
            const conversationActions = document.createElement('div');
            conversationActions.classList.add('conversation-actions');
            ...
            conversationActions.appendChild(renameBtn);
            ...
            conversationItem.addEventListener('click', function() {
                loadConversationToChat(conversation.id);
    ...
    function showRenameModal(id, currentTitle) {
        ...
        saveNameBtn.onclick = function() {
            ...
            if (newTitle) {
                const conversation = loadConversation(id);
                saveConversation(id, newTitle, conversation.messages);
                showConversations();
    ...
    function initCodeEditor() {
        ...
        // Save code button click event
        document.getElementById('save-code-btn').addEventListener('click', function() {
            const code = codeEditor.getValue();
            const filename = document.getElementById('code-filename').value.trim() || 'code.js';
            const language = document.getElementById('code-language').value;
            
            if (code) {
                // Add code to chat as a message
                const codeMessage = `Here's the code (${filename}):\n\n\`\`\`${language}\n${code}\n\`\`\``;
                
                messageInput.value = codeMessage;
                sendMessage();
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    // Display location
                    document.getElementById('location-coords').textContent = `Latitude: ${userLocation.lat.toFixed(6)}, Longitude: ${userLocation.lng.toFixed(6)}`;
                ...
                function(error) {
                    ...
                    document.getElementById('location-coords').innerHTML = `<span class="location-error">Error getting location: ${error.message}</span>`;
                    document.getElementById('location-display').style.display = 'block';
                }
            );
        } else {
            document.getElementById('location-coords').innerHTML = '<span class="location-error">Geolocation is not supported by this browser.</span>';
            document.getElementById('location-display').style.display = 'block';
        }
    }
    ...
    function speakText(text) {
        ...
        if (ttsVoice) {
            speech.voice = ttsVoice;
        }
        
        // Show TTS indicator
        const ttsIndicator = document.createElement('div');
        ttsIndicator.classList.add('tts-indicator');
        ttsIndicator.innerHTML = 'Speaking <button class="tts-stop-btn"><i class="fas fa-stop"></i></button>';
        document.body.appendChild(ttsIndicator);
        
        // Stop button click event
        ttsIndicator.querySelector('.tts-stop-btn').addEventListener('click', function() {
            window.speechSynthesis.cancel();
            document.body.removeChild(ttsIndicator);
        });
        
        // Speech end event
        speech.onend = function() {
            if (document.body.contains(ttsIndicator)) {
                document.body.removeChild(ttsIndicator);
            }
            currentSpeech = null;
        };
    ...
    function showModal(modalId) {
        ...
        if (modalId === 'chat-history-modal') {
            showConversations();
        } else if (modalId === 'code-editor-modal') {
            // Initialize code editor if not already done
            if (!codeEditor) {
                initCodeEditor();
            } else {
                // Refresh editor to fix rendering issues
                codeEditor.refresh();
            }
        }
    }
    ...
    micBtn.addEventListener('click', function() {
        ...
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            showToast('Speech recognition is not supported by this browser.');
        }
    });
    ...
    codeEditorBtn.addEventListener('click', function() {
        showModal('code-editor-modal');
    });
    
    // Knowledge base button click
    knowledgeBaseBtn.addEventListener('click', function() {
        showModal('knowledge-base-modal');
    });
    
    // Reasoning button click
    reasoningBtn.addEventListener('click', function() {
        showModal('reasoning-modal');
    });
    
    // Web URL button click
    webUrlBtn.addEventListener('click', function() {
        showModal('web-url-modal');
    });
    ...
    // AI features button click
    aiFeaturesBtn.addEventListener('click', function() {
        showModal('ai-features-modal');
    });
    ...
    // Minimize thinking box button click
    minimizeThinkingBtn.addEventListener('click', function() {
        thinkingBox.style.display = 'none';
    });
    
    // Minimize terminal box button click
    minimizeTerminalBtn.addEventListener('click', function() {
        terminalBox.style.display = 'none';
    });
    
    // Clear terminal button click
    ...
    document.getElementById('dark-mode-toggle').addEventListener('change', function() {
        ...
        // Update code editor theme if initialized
        if (codeEditor) {
            codeEditor.setOption('theme', darkMode ? 'dracula' : 'default');
        }
    ...
    // Terminal calls toggle
    document.getElementById('terminal-calls-toggle').addEventListener('change', function() {
        terminalCallsEnabled = this.checked;
        
        if (terminalCallsEnabled) {
    ...
    // Memory mode button click
    document.getElementById('memory-mode-btn').addEventListener('click', function() {
    ...
    // Text size input change
    document.getElementById('text-size-input').addEventListener('change', function() {
        textSize = parseInt(this.value);
        
        // Clamp value between 60 and 200
    ...
    document.querySelectorAll('input[name="distance-unit"]').forEach(function(radio) {
        radio.addEventListener('change', function() {
    ...
    // Location access button click
    document.getElementById('location-access-btn').addEventListener('click', function() {
        getUserLocation();
    });
    
    // System prompt save button click
    ...
    // MCP tabs
    document.querySelectorAll('.mcp-tab-btn').forEach(function(tab) {
        tab.addEventListener('click', function() {
            const tabName = this.dataset.tab;
            
            // Remove active class from all tabs
            document.querySelectorAll('.mcp-tab-btn').forEach(function(t) {
                t.classList.remove('active');
            });
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.tool-expand-btn')) {
            const btn = e.target.closest('.tool-expand-btn');
    ...
    // MCP expand buttons
    document.addEventListener('click', function(e) {
        if (e.target.closest('.mcp-expand-btn')) {
            const btn = e.target.closest('.mcp-expand-btn');
            const mcpItem = btn.closest('.mcp-item');
            const mcpDetails = mcpItem.querySelector('.mcp-details');
            
            if (mcpDetails.style.display === 'none') {
                mcpDetails.style.display = 'block';
                btn.innerHTML = '<i class="fas fa-chevron-up"></i>';
                mcpItem.classList.add('expanded');
            } else {
                mcpDetails.style.display = 'none';
                btn.innerHTML = '<i class="fas fa-chevron-down"></i>';
                mcpItem.classList.remove('expanded');
            }
        }
    });
    
    // Web URL load button click
    document.getElementById('load-url-btn').addEventListener('click', function() {
        ...
        if (url) {
            ...
            // Simulate loading URL (replace with actual API call)
            setTimeout(function() {
                // Reset button
                document.getElementById('load-url-btn').textContent = 'Load';
                document.getElementById('load-url-btn').disabled = false;
                
                // Show preview
                document.getElementById('url-thumbnail').src = 'https://via.placeholder.com/300x200?text=Website+Preview';
                document.getElementById('url-title').textContent = url;
                document.getElementById('url-preview-container').style.display = 'block';
            }, 1000);
        }
    });
    ...
    document.getElementById('extract-data-btn').addEventListener('click', function() {
        ...
        if (extractOptions.style.display === 'none') {
    ...
    document.querySelectorAll('.extract-option').forEach(function(option) {
        option.addEventListener('click', function() {
            const type = this.dataset.type;
            
            // Remove active class from all options
            document.querySelectorAll('.extract-option').forEach(function(opt) {
            ...
            // Hide extract options
            document.getElementById('extract-options').style.display = 'none';
            
            // Show extracted data container
            document.getElementById('extracted-data-container').style.display = 'block';
            
            // Update extracted data type
            document.getElementById('extracted-data-type').textContent = `Extracted ${type.charAt(0).toUpperCase() + type.slice(1)}`;
            
            // Show appropriate content based on type
            if (type === 'text') {
                document.getElementById('extracted-text-content').style.display = 'block';
                document.getElementById('extracted-images-container').style.display = 'none';
                document.getElementById('extracted-text-content').textContent = 'This is the extracted text from the webpage. It would contain all the textual content from the page.';
            } else if (type === 'images') {
                document.getElementById('extracted-text-content').style.display = 'none';
                document.getElementById('extracted-images-container').style.display = 'block';
                document.getElementById('current-extracted-image').src = 'https://via.placeholder.com/400x300?text=Extracted+Image';
            } else if (type === 'code') {
                document.getElementById('extracted-text-content').style.display = 'block';
                document.getElementById('extracted-images-container').style.display = 'none';
                document.getElementById('extracted-text-content').textContent = '<!DOCTYPE html>\n<html>\n<head>\n    <title>Page Title</title>\n</head>\n<body>\n    <h1>This is a heading</h1>\n    <p>This is a paragraph.</p>\n</body>\n</html>';
            }
        });
    });
    
    // Copy extracted data button click
    document.getElementById('copy-extracted-data').addEventListener('click', function() {
        const extractedText = document.getElementById('extracted-text-content').textContent;
    ...
    document.getElementById('prev-image').addEventListener('click', function() {
        // Implement previous image functionality
    });
    
    document.getElementById('next-image').addEventListener('click', function() {
        // Implement next image functionality
    });
    
    // Download image button click
    document.getElementById('download-image').addEventListener('click', function() {
        const imageUrl = document.getElementById('current-extracted-image').src;
        
        if (imageUrl) {
            const a = document.createElement('a');
            a.href = imageUrl;
            a.download = 'extracted-image.jpg';
    ...
    // Text to image feature button click
    document.getElementById('txt2img-feature-btn').addEventListener('click', function() {
        showModal('txt2img-modal');
        closeModal('ai-features-modal');
    });
    
    // Image to text feature button click
    document.getElementById('img2txt-feature-btn').addEventListener('click', function() {
        showModal('image-upload-modal');
        closeModal('ai-features-modal');
    });
    
    // Text to speech feature button click
    document.getElementById('txt2speech-feature-btn').addEventListener('click', function() {
        ttsBtn.style.display = 'block';
        closeModal('ai-features-modal');
    });
    
    // Generate image button click
    document.getElementById('generate-img-btn').addEventListener('click', function() {
        ...
        if (prompt) {
            ...
            setTimeout(function() {
                // Reset button
                document.getElementById('generate-img-btn').textContent = 'Generate';
                document.getElementById('generate-img-btn').disabled = false;
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    img.src = `https://via.placeholder.com/${size}?text=Generated+Image+${i+1}`;
    ...
    // New agent button click
    document.getElementById('new-agent-btn').addEventListener('click', function() {
        showModal('new-agent-modal');
    });
    
    // Save new agent button click
    document.getElementById('save-new-agent-btn').addEventListener('click', function() {
        const agentCalling = document.getElementById('agent-calling').value.trim();
        ...
        if (agentCalling && agentName) {
            // Create new agent
            const newAgent = {
                id: generateId(),
                calling: agentCalling,
                name: agentName,
            ...
            installedAgents.push(newAgent);
            
            // Add to activated agents
            activatedAgents.push(newAgent.id);
    ...
    // Cancel new agent button click
    document.getElementById('cancel-new-agent-btn').addEventListener('click', function() {
        closeModal('new-agent-modal');
    });
    
    // Activate reasoning button click
    document.getElementById('activate-reasoning').addEventListener('click', function() {
        reasoningEnabled = !reasoningEnabled;
        
        if (reasoningEnabled) {
            this.textContent = 'Deactivate Reasoning';
            this.classList.add('active');
        } else {
            this.textContent = 'Activate Reasoning';
            this.classList.remove('active');
        }
    ...
    document.getElementById('reasoning-model').addEventListener('change', function() {
        reasoningModel = this.value;
        
        // Save preference
        localStorage.setItem('reasoningModel', reasoningModel);
    });
    
    // Resize handle for message input
    ...
    function resizeInput(e) {
        ...
        // Clamp height between 60px and 300px
