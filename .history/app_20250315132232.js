...
async function sendSimpleMessage(message, apiOptions) {
    ...
    try {
        ...
        // Create a simple message array without system messages
        const messages = [
...
function updateConversationsList() {
    // Since we removed the sidebar, we'll just update the current conversation title
    if (currentConversationId) {
        const conversation = conversations.find(c => c.id === currentConversationId);
        if (conversation) {
            currentChatTitle.textContent = conversation.title;
