...
function initApp() {
    ...
    document.getElementById('img2txt-feature-btn').addEventListener('click', openUploadModal);
    document.getElementById('txt2speech-feature-btn').addEventListener('click', convertLastMessageToSpeech);
...
// Handle message input for tool suggestions
...
function initializeTextSizeSlider() {
    ...
    const savedSize = localStorage.getItem('text-size') || '16';
...
function updateTextSizeFromInput(e) {
    ...
    // Validate the input
    if (size < 10) e.target.value = 10;
    if (size > 24) e.target.value = 24;
