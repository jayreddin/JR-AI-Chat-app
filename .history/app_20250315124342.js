...
function checkAuthStatus() {
    ...
    try {
        ...
        if (typeof puter.auth.signIn === 'function') {
            ...
            if (typeof puter.auth.isLoggedIn === 'function') {
                puter.auth.isLoggedIn()
                    .then(isLoggedIn => {
                        console.log('Auth status (v2):', isLoggedIn ? 'Logged in' : 'Not logged in');
                        ...
                        if (isLoggedIn) {
                            ...
                            if (typeof puter.auth.getUser === 'function') {
                                ...
                                // Create a minimal user object if we can't get details
                    ...
                    .catch(error => {
                        console.error('Auth check error (v2):', error);
                        updateAuthUI(false);
                    });
            } else {
                console.warn('isLoggedIn method not available in Puter API v2');
                updateAuthUI(false);
            }
        }
        // Try v1 API methods
...
function ensureAppDirectory() {
    if (puter.fs && typeof puter.fs.mkdir === 'function') {
        puter.fs.mkdir('/apps')
            .then(() => puter.fs.mkdir('/apps/ai-chat'))
            .then(() => {
                console.log('App directory created or already exists');
            })
            .catch(error => {
        ...
        puter.fs.read(`/apps/ai-chat/conversations_${currentUser.username}.json`)
            .then(data => {
                ...
                try {
                    ...
                    // We already loaded from localStorage as fallback
                }
            })
            .catch(error => {
