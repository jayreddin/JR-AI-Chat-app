...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof puter === 'undefined') {
        ...
        window.puter = {
            ai: {
                chat: async function() {
                    return { message: { content: "Puter API not available. This is a fallback response." } };
                },
                ...
                txt2speech: async function() { throw new Error("Text to speech not available"); }
            },
            auth: {
                ...
                login: async function() { throw new Error("Puter API not available"); },
                signIn: async function() { throw new Error("Puter API not available"); },
                logout: async function() { return true; },
                signOut: async function() { return true; }
            },
            fs: {
                mkdir: async function() { return true; },
                write: async function() { return true; },
                read: async function() { throw new Error("File not found"); }
            }
        };
    }
...
function login() {
    ...
    try {
        ...
        if (typeof puter.auth.login === 'function') {
            ...
            console.error('Login function not available in Puter API');
            alert('Error: Login function not available. The API might have changed or not fully loaded.');

            // Try to use a different method if available
            if (typeof puter.auth.signIn === 'function') {
                console.log('Trying signIn method instead...');
                puter.auth.signIn()
                    .then(user => {
...
function logout() {
    ...
    try {
        ...
        if (typeof puter.auth.logout === 'function') {
            ...
            console.log('Using signOut method instead...');
            puter.auth.signOut()
                .then(() => {
